                                   ...
                                   esp_lcd_panel_handle_t *ret_panel)
{
    ...
    if (i2c_bus_handle) {
        i2c_master_dev_handle_t pwr_dev = NULL;
        i2c_device_config_t dev_cfg = {
            .dev_addr_length = I2C_ADDR_BIT_LEN_7,
            .device_address = 0x45,
            .scl_speed_hz = 100000,
        };
        if (i2c_master_bus_add_device(i2c_bus_handle, &dev_cfg, &pwr_dev) == ESP_OK && pwr_dev) {
            uint8_t tx_buf[2] = {0};
            tx_buf[0] = 0x95; tx_buf[1] = 0x11;
            i2c_master_transmit(pwr_dev, tx_buf, sizeof(tx_buf), 50);
            tx_buf[0] = 0x95; tx_buf[1] = 0x17;
            i2c_master_transmit(pwr_dev, tx_buf, sizeof(tx_buf), 50);
            tx_buf[0] = 0x96; tx_buf[1] = 0x00;
            i2c_master_transmit(pwr_dev, tx_buf, sizeof(tx_buf), 50);
            vTaskDelay(pdMS_TO_TICKS(100));
            tx_buf[0] = 0x96; tx_buf[1] = 0xFF;
            i2c_master_transmit(pwr_dev, tx_buf, sizeof(tx_buf), 50);

            i2c_master_bus_rm_device(pwr_dev);
        } else {
            ESP_LOGW(TAG, "Failed to add I2C device 0x45 on shared bus");
        }
    } else {
        ESP_LOGW(TAG, "Shared I2C bus handle is NULL, skip 0x45 setup");
    }
